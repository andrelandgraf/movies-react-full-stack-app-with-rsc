...
import type { FilteredMovie, FilteredMoviesAPIConfiguration } from './movies-api.server';
import { useMoviesImageProps } from './useMoviesBasePath';

type MovieCardProps = {
  movie?: FilteredMovie;
  moviesConfig?: FilteredMoviesAPIConfiguration;
};

export function MovieCard({ movie, moviesConfig }: MovieCardProps) {
  const imageProps = useMoviesImageProps(moviesConfig, movie?.poster_path, 220);
  if (!movie) {
    return (
    ...
    <section className="w-full flex flex-col gap-4">
      ...
      >
        {isLoading
          ...
          : movies.map((movie) => <MovieCard key={movie.id} movie={movie} moviesConfig={moviesConfig} />)}
      </ul>
    </section>
  );
}
